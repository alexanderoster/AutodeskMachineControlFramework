...
protected:
	...
	// bCheckError == false suppresses the per-call n_get_last_error query. That query is a control
	// command, and on RTC6 Ethernet boards every control command forces the DLL to wait until the
